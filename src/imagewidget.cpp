...
void BrainSuiteWindow::setZoomFactor(float f)
{
  imageState.zoomf = f;//(float)pow(ImageWidget::zoomMultiplier,(float)imageState.zoom);
  imageState.zoom=(std::log(imageState.zoomf)/std::log(ImageWidget::zoomMultiplier));
//qDebug()<<imageState.zoomf<<'\t'<<imageState.zoom;
...
bool ImageWidget::event(QEvent *event)
{
  if (event->type()==QEvent::NativeGesture)
  {
    ...
    if (gesture)
    {
      static float zoomf=1.0f;
      static float startingzoom=imageState.zoomf;
      if(gesture->gestureType() == Qt::BeginNativeGesture)
      {
        zoomf=1.0f;
        startingzoom=imageState.zoomf;
      }
      if(gesture->gestureType() == Qt::ZoomNativeGesture)
      {
        zoomf*=(1.0f+float(gesture->value()));
        brainSuiteWindow->setZoomFactor(startingzoom*zoomf);
        update();
      }
      // qDebug()<<"(gestures wildly) "<<gesture->gestureType()<<" "<<gesture->value()<<" "<<zoomf;
    }
  }
...
ImageState::ImageState() :
  ...
  zoom(0), zoomf(1.0f),
  smoothImage(false),
  viewOverlay(true), viewOverlay2(true),
  makeOverlay1BackgroundClear(false), makeOverlay2BackgroundClear(false),
  viewMask(true), viewLabels(true), viewInfo(true), viewVolume(true),
...
void integrateXZ(DSImage &bitmap, const Vol3D<T> &vol, const size_t y, Operation operation)
{
  ...
  else
  {
    ...
    for (size_t z=0;z<cz;z++)
    {
      ...
      {
          operation(*pixel++,vol(x,y,z));// integrate over y!
      }
    }
  }
}
...
class Integrator {
public:
  static void integrateYZ(DSImage &bitmap, const Vol3D<T> &vol, const size_t x, Operation operation)
  {
    Timer t;
    t.start();
    bitmap.resize(vol.cx,vol.cy);
    QRgb* pixel = reinterpret_cast<QRgb*>(bitmap.image.bits());
    const auto cx = vol.cx;
    const auto cy = vol.cy;
    const auto cz = vol.cz;
      for (size_t y=0;y<cy;y++)
        for (size_t x=0;x<cx;x++)
        {
          ...
          operation(*pixel++,a); // integrate over z!
        }
      t.stop();
      qDebug()<<"integration took "<<t.ns();
...
class IntegratorND {
public:
  static void integrateUAxis(DSImage &bitmap, const VolND<T> &vol, const int t, Operation operation) // specify the time slice
  {
    Timer timer;
    timer.start();
    bitmap.resize(vol.nx(),vol.ny());
    QRgb* pixel = reinterpret_cast<QRgb*>(bitmap.image.bits());
    const auto cx = vol.nx();
    const auto cy = vol.ny();
    const auto cz = vol.nz();
//    const auto ct = vol.nt();
      for (size_t y=0;y<cy;y++)
        for (size_t x=0;x<cx;x++)
        {
          float64 a=0;
          ...
          operation(*pixel++,a/cz); // integrate over z!
        }
      timer.stop();
//      qDebug()<<"z integration took "<<timer.ns();
  }
  static void integrateVAxis(DSImage &bitmap, const VolND<T> &vol, const int u, Operation operation) // specify the time slice
  {
    Timer timer;
    timer.start();
    bitmap.resize(vol.nx(),vol.ny());
    ...
    const auto cx = vol.nx();
    const auto cy = vol.ny();
    const auto cv = vol.nv();
    const int z=0; // TODO: add parameter for point/orientation
      for (size_t y=0;y<cy;y++)
        for (size_t x=0;x<cx;x++)
        {
          float64 a=0;
          for (size_t v=0;v<cv;v++)
          {
            a+=vol(x,y,z,u,v);
          }
          operation(*pixel++,a/cv); // integrate over z!
        }
      timer.stop();
//      qDebug()<<"t integration took "<<timer.ns();
  }
};


template <class T, class Operation>
class LabelIntegratorND {
public:
  static void integrateOverLabel(DSImage &bitmap, const VolND<T> &vol, const SliceT<uint16> &sLabel, int labelID, Operation &operation) // specify the time slice
  {
    Timer timer;
    timer.start();
    bitmap.resize(vol.nx(),vol.ny());
    ...
    const auto cy = vol.ny();
    const auto cz = vol.nz();
    const auto cu = vol.nu();
    const auto cv = vol.nv();
//    const auto ct = vol.nt();
    SliceT<uint16> sMask;
    sMask.setsize(sLabel.cx,sLabel.cy);
    const auto np=sLabel.cx*sLabel.cy;
    for (size_t i=0;i<np;i++) sMask[i]=sLabel[i]==labelID ? 1 : 0;
    float nx=0;
    for (auto i=0;i<np;i++) nx+=sMask[i];
    qDebug()<<"integrating "<<nx<<"points for label "<<labelID;
    const size_t z=1;
    float amax=-10000;
    size_t n=0;
    for (size_t x=0;x<cx;x++)
    {
      ...
      {
        float32 a=0;
        for (size_t v=v;v<cv;v++)
          for (size_t u=0;u<cu;u++)
          {
            a+=/*sMask(u,v)**/vol(x,y,z,u,v);
            n++;
          }
        if (a>amax) amax=a;
        operation(*pixel++,a/nx); // integrate over z!
      }
    }
    timer.stop();
    qDebug()<<"spectral integration took "<<timer.ns()<<" with max "<<amax<<" over "<<n<<" data";
  }
};

template <class RenderOp, class Type>
bool ImageWidget::integrateVolumeT(DSImage &targetBitmap,
                  ...
                  RenderOp &op, DSPoint scaleFactor, Orientation::Code orientation)
{
  ...
  switch (orientation)
  {
    ...
    case Orientation::XZ :
    {
      ...
      if (brainSuiteWindow->toolManager.relaxationToolboxForm && brainSuiteWindow->toolManager.relaxationToolboxForm->vSpectral
          && brainSuiteWindow->toolManager.relaxationToolboxForm->integration())
      {
        int u=brainSuiteWindow->toolManager.relaxationToolboxForm->spectralPosition.u;
        auto vSpectral=dynamic_cast<VolND<Type> *>(brainSuiteWindow->toolManager.relaxationToolboxForm->vSpectral.get());
        if (vSpectral)
        {
          IntegratorND<Type,RenderOp>::integrateUAxis(targetBitmap,*vSpectral,u,op);
        }
        else
          renderXZ(targetBitmap,volume,slice,op);
      }
      else
        renderXZ(targetBitmap,volume,slice,op);
      break;
    ...
    case Orientation::YZ :
    {
      ...
      if (brainSuiteWindow->toolManager.relaxationToolboxForm && brainSuiteWindow->toolManager.relaxationToolboxForm->vSpectral
          && brainSuiteWindow->toolManager.relaxationToolboxForm->integration()
          && brainSuiteWindow->toolManager.relaxationToolboxForm->vSpectral->dim[3]>0)
      {
        int v=brainSuiteWindow->toolManager.relaxationToolboxForm->spectralPosition.v;
        auto vSpectral=dynamic_cast<VolND<Type> *>(brainSuiteWindow->toolManager.relaxationToolboxForm->vSpectral.get());
        if (vSpectral)
        {
          IntegratorND<Type,RenderOp>::integrateVAxis(targetBitmap,*vSpectral,v,op);
        }
//      Integrator<Type,RenderOp>::integrateYZ(targetBitmap,volume,slice,op); break;
      }
      else
        renderYZ(targetBitmap,volume,slice,op);
    }
    default:
        return false;
  }
...
template <class RenderOp, class Type>
bool ImageWidget::integrateVolumeLabelT(DSImage &targetBitmap,
                  const Vol3D<Type> &volume,  const SliceT<uint16> &sLabel, int labelID,
  ...
  switch (orientation)
  {
    ...
    case Orientation::XZ :
    {
          ...
          && brainSuiteWindow->toolManager.relaxationToolboxForm->integration())
      {
        ...
        if (vSpectral)
        {
          LabelIntegratorND<Type,RenderOp>::integrateOverLabel(targetBitmap,*vSpectral,sLabel,1,op);
        }
        else
          renderXZ(targetBitmap,volume,slice,op);
      }
    ...
    case Orientation::YZ :
    {
          ...
          && brainSuiteWindow->toolManager.relaxationToolboxForm->vSpectral->dim[3]>0)
      {
        ...
        if (vSpectral)
        {
          LabelIntegratorND<Type,RenderOp>::integrateOverLabel(targetBitmap,*vSpectral,sLabel,2,op);
        }
//      Integrator<Type,RenderOp>::integrateYZ(targetBitmap,volume,slice,op); break;
      }
      else
...
bool ImageWidget::integrate(DSImage &targetBitmap,
                         ...
                         RenderOp &op, DSPoint scaleFactor)
{
  ...
  {
    case SILT::Sint16 : integrateVolumeT(targetBitmap,*static_cast<const Vol3D<sint16> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Uint16 : integrateVolumeT(targetBitmap,*static_cast<const Vol3D<uint16> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Float32: integrateVolumeT(targetBitmap,*static_cast<const Vol3D<float32> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Float64: integrateVolumeT(targetBitmap,*static_cast<const Vol3D<float64> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Uint8  : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint8> *>(volume),position,op,scaleFactor,orientation);  break;
    case SILT::Sint32 :
    case SILT::Uint32 : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint32> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::RGB8   : renderVolumeT(targetBitmap,*static_cast<const Vol3D<rgb8> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Eigensystem3x3f :
      renderVolumeT(targetBitmap,*static_cast<const Vol3D<EigenSystem3x3f> *>(volume),position,op,scaleFactor,orientation); break;
    default:
...
template <class RenderOp>
bool ImageWidget::integrateLabel(DSImage &targetBitmap, const Vol3DBase *volume, const SliceT<uint16> &sLabel, int labelID,
                        const IPoint3D position, RenderOp &op, DSPoint scaleFactor)
{
  if (!volume) return false;
  switch (volume->typeID())
  {
    case SILT::Sint16 : integrateVolumeLabelT(targetBitmap,*static_cast<const Vol3D<sint16> *>(volume),sLabel,labelID,position,op,scaleFactor,orientation); break;
    case SILT::Uint16 : integrateVolumeLabelT(targetBitmap,*static_cast<const Vol3D<uint16> *>(volume),sLabel,labelID,position,op,scaleFactor,orientation); break;
    case SILT::Float32: integrateVolumeLabelT(targetBitmap,*static_cast<const Vol3D<float32> *>(volume),sLabel,labelID,position,op,scaleFactor,orientation); break;
    case SILT::Float64: integrateVolumeLabelT(targetBitmap,*static_cast<const Vol3D<float64> *>(volume),sLabel,labelID,position,op,scaleFactor,orientation); break;
    case SILT::Uint8  : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint8> *>(volume),position,op,scaleFactor,orientation);  break;
    case SILT::Sint32 :
    case SILT::Uint32 : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint32> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::RGB8   : renderVolumeT(targetBitmap,*static_cast<const Vol3D<rgb8> *>(volume),position,op,scaleFactor,orientation); break;
                         ...
                         RenderOp &op, DSPoint scaleFactor)
{
  ...
  {
    case SILT::Uint8  : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint8> *>(volume),position,op,scaleFactor,orientation);  break;
    case SILT::Sint16 : renderVolumeT(targetBitmap,*static_cast<const Vol3D<sint16> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Uint16 : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint16> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Sint32 :
    case SILT::Uint32 : renderVolumeT(targetBitmap,*static_cast<const Vol3D<uint32> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Float32: renderVolumeT(targetBitmap,*static_cast<const Vol3D<float32> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Float64: renderVolumeT(targetBitmap,*static_cast<const Vol3D<float64> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::RGB8   : renderVolumeT(targetBitmap,*static_cast<const Vol3D<rgb8> *>(volume),position,op,scaleFactor,orientation); break;
    case SILT::Eigensystem3x3f :
      renderVolumeT(targetBitmap,*static_cast<const Vol3D<EigenSystem3x3f> *>(volume),position,op,scaleFactor,orientation); break;
    default:
...
{
  if (dst.sizeInBytes()!=src.sizeInBytes()) return false;
...
void ImageWidget::initPrimaryBitmap()
{
  auto *volume=brainSuiteWindow->brainSuiteDataManager.volume1();
  if (!volume) return;
  const auto scale = std::min(volume->rx,std::min(volume->ry,volume->rz));
  switch (orientation)
  {
    case Orientation::XY :
    {
      volumeBitmap.resize(volume->cx,volume->cy);
      volumeBitmap.setRes(volume->rx,volume->ry,scale);
    }
    case Orientation::XZ :
    {
      volumeBitmap.resize(volume->cx,volume->cz);
      volumeBitmap.setRes(volume->rx,volume->rz,scale);
    }
    case Orientation::YZ :
    {
      volumeBitmap.resize(volume->cy,volume->cz);
      volumeBitmap.setRes(volume->ry,volume->rz,scale);
    }
...
void ImageWidget::blendLayers()
{
  ...
  {
    if (imageState.viewVolume && brainSuiteWindow->brainSuiteDataManager.volume1())
    {
      drawnYet=true;
  ...
  if (imageState.viewOverlay && brainSuiteWindow->brainSuiteDataManager.volume2())
  {
    ...
    {
      if (!imageState.makeOverlay1BackgroundClear)
        imageOverlay1Bitmap.setAlpha(drawnYet ? imageState.overlayAlpha : 255);
      else
      {
        auto *lut = brainSuiteWindow->imageOverlay1LUT;
        uint32_t blank = (lut) ? lut[128] : 0xFFFFFF;
        imageOverlay1Bitmap.alphaMask(drawnYet ? imageState.overlayAlpha : 255, blank);
      }
  ...
  if (imageState.viewOverlay2 && brainSuiteWindow->brainSuiteDataManager.volume3())
  {
    ...
    {
      if (!imageState.makeOverlay2BackgroundClear)
        imageOverlay2Bitmap.setAlpha(drawnYet ? imageState.overlay2Alpha : 255);
      else
      {
        auto *lut = brainSuiteWindow->imageOverlay2LUT;
        uint32_t blank = (lut) ? lut[128] : 0xFFFFFF;
        imageOverlay2Bitmap.alphaMask(drawnYet ? imageState.overlay2Alpha : 255, blank);
      }
      if (drawnYet) blendPainter.setCompositionMode(QPainter::CompositionMode_SourceOver);
      blendPainter.drawImage(targetRect,imageOverlay2Bitmap.image);
    }
  ...
  if (imageState.viewLabels && brainSuiteWindow->brainSuiteDataManager.labelVolume())
  {
  ...
  if (imageState.viewMask && brainSuiteWindow->brainSuiteDataManager.mask().size())
  {
...
void ImageWidget::drawCurveXY(QPainter *painter, std::vector<DSPoint> &points)
{
  ...
  if (np>1)
  {
    ...
    float rz = (brainSuiteWindow->brainSuiteDataManager.volume1()) ? brainSuiteWindow->brainSuiteDataManager.volume1()->rz : 1;
    float wz = (z  - 0.5f) * rz;
...
void ImageWidget::drawCurveXZ(QPainter *painter, std::vector<DSPoint> &points)
{
  ...
  if (np>1)
  {
    ...
    float ry = (brainSuiteWindow->brainSuiteDataManager.volume1()) ? brainSuiteWindow->brainSuiteDataManager.volume1()->ry : 1;
    float wy = (y  - 0.5f) * ry;
...
void ImageWidget::drawCurveYZ(QPainter *painter, std::vector<DSPoint> &points)
{
  ...
  if (np>1)
  {
    ...
    // clip based on Y
    float rx = (brainSuiteWindow->brainSuiteDataManager.volume1()) ? brainSuiteWindow->brainSuiteDataManager.volume1()->rx : 1;
    float wx = (x  - 0.5f) * rx;
...
//    painter.setCompositionMode(QPainter::RasterOp_SourceXorDestination);
    painter.setCompositionMode(QPainter::RasterOp_SourceXorDestination);
    QPen pen(QColor(/*active ? 0xFFFF00 : 0x0000FF*/0xFFFFFF), 0.25, Qt::DashLine);
...
//    pen.setDashPattern(dashes);
  ...
  if (false && brainSuiteWindow->brainSuiteDataManager.protocolCurveSet)
  {
    if (BrainSuiteSettings::showCurves && BrainSuiteSettings::showCurvesOnImageSlices)
    {
      ...
      ProtocolCurveSet *cp = brainSuiteWindow->brainSuiteDataManager.protocolCurveSet.get();
...
IPoint3D scaledRenderPoint(const Vol3DBase *vol, const Vol3DBase *vOverlay, const Orientation::Code orientation, IPoint3D p)
{              
...
bool ImageWidget::renderIntegratedLabeledVolume(DSImage &targetBitmap, const uint32 *LUT,
                                                const Vol3DBase *volume,
                                             const SliceT<uint16> &sLabel,
                                               const ImageScale &brightnessScale,
                                                const IPoint3D position)
{
  if (!volume) return false;
  if (brightnessScale.minv!=0)
  {
    RangeLUTOp op(LUT,brightnessScale.minv,brightnessScale.maxv);
    //DSImage &targetBitmap, const Vol3DBase *volume, SliceT<uint16> &sLabel, int labelID, const IPoint3D position, RenderOp &op, DSPoint scaleFactor
    return integrateLabel(targetBitmap,volume,sLabel,1,position,op,DSPoint(1,1,1));
  }
  if (volume->typeID()==SILT::Eigensystem3x3f && BrainSuiteSettings::useColorFA)
  {
    ColorFAOp op((float)brightnessScale.maxv);
...
bool ImageWidget::renderIntegratedScaledVolume(DSImage &targetBitmap, const uint32 *LUT,
                                     const Vol3DBase *volume, const ImageScale &brightnessScale,
                                     const IPoint3D position)
{
  if (!volume) return false;
  ...
  {
    RangeLUTOp op(LUT,brightnessScale.minv,brightnessScale.maxv);
    return integrate(targetBitmap,volume,position,op,DSPoint(1,1,1));
  }
  if (volume->typeID()==SILT::Eigensystem3x3f && BrainSuiteSettings::useColorFA)
  {
    ColorFAOp op((float)brightnessScale.maxv);
...
bool ImageWidget::renderVolume(const Vol3DBase *volume, const IPoint3D position)
{
  ...
  if (true)
  {
    // if (brainSuiteWindow->toolManager.relaxationToolboxForm
    //     &&brainSuiteWindow->toolManager.relaxationToolboxForm->spectralLabels.slicesize>0)
    // {
    //   renderIntegratedLabeledVolume(volumeBitmap,volumeLUT,volume,
    //                                brainSuiteWindow->toolManager.relaxationToolboxForm->spectralLabels,imageState.brightness,position);
    // }
    // else
      return renderIntegratedScaledVolume(volumeBitmap,volumeLUT,volume,imageState.brightness,position);
  }
  else
...
void ImageWidget::updateImage()
{
  if (dirty)
  {
    if (brainSuiteWindow)
    {
      const Vol3DBase *volume = brainSuiteWindow->brainSuiteDataManager.volume1();
      const Vol3DBase *overlay1 = brainSuiteWindow->brainSuiteDataManager.volume2();
      const Vol3DBase *overlay2 = brainSuiteWindow->brainSuiteDataManager.volume3();
      renderVolume(brainSuiteWindow->brainSuiteDataManager.volume1(),brainSuiteWindow->currentVolumePosition);
      IPoint3D overlay1Point = scaledRenderPoint(volume,overlay1,orientation,brainSuiteWindow->currentVolumePosition);
      renderScaledVolume(imageOverlay1Bitmap,imageState.threeChannel ? ColorMap::greenLUT : brainSuiteWindow->imageOverlay1LUT,overlay1,imageState.overlay1Brightness,overlay1Point);
      IPoint3D overlay2Point = scaledRenderPoint(volume,overlay2,orientation,brainSuiteWindow->currentVolumePosition);
      renderScaledVolume(imageOverlay2Bitmap,imageState.threeChannel ? ColorMap::blueLUT : brainSuiteWindow->imageOverlay2LUT,overlay2,imageState.overlay2Brightness,overlay2Point);
      IPoint3D labelPoint = scaledRenderPoint(volume,brainSuiteWindow->brainSuiteDataManager.labelVolume(),orientation,brainSuiteWindow->currentVolumePosition);
      LabelOp labelOp;
      render(labelBitmap,brainSuiteWindow->brainSuiteDataManager.labelVolume(),labelPoint,labelOp,DSPoint(1,1,1));
      if (imageState.viewLabelOutline)
        stencilLabelOutline(labelOutlineBitmap,labelBitmap);
      if (brainSuiteWindow->brainSuiteDataManager.mask().size())
      {
        IPoint3D maskPoint = scaledRenderPoint(volume,&brainSuiteWindow->brainSuiteDataManager.mask(),orientation,brainSuiteWindow->currentVolumePosition);
        RawOp nop;
        renderVolumeT(maskBitmap,brainSuiteWindow->brainSuiteDataManager.mask(),maskPoint,nop,DSPoint(1,1,1),orientation);
...
void ImageWidget::mousePressEvent(QMouseEvent *event)
{
  ...
  {
    if (brainSuiteWindow->contextMenu)
      brainSuiteWindow->contextMenu->exec(event->globalPos());
  }
  else
  {
...
float ImageWidget::zoomBestFit()
{
  ...
  QSize sz=volumeBitmap.size();
  float width_mm=sz.width()*rx;
  float height_mm=sz.height()*ry;
  // qDebug()<<"ZBF "<<rx<<" x "<<ry<<" : "<<sz;
  float zoomX = imageFrame->imagePaneSize.width() / width_mm;
  float zoomY = imageFrame->imagePaneSize.height() / height_mm;
//  qDebug()<<"pane: "<<imageFrame->isVisible()<<" "<<imageFrame->imagePaneSize<<" size(mm): "<<width_mm<<"x"<<height_mm<<" zx,zy: "<<zoomX<<","<<zoomY<<" -> "<<std::min(zoomX,zoomY)
//         <<" "<<std::min(zoomX,zoomY)*volumeBitmap.size().width()<<" x "<<std::min(zoomX,zoomY)*volumeBitmap.size().height();
